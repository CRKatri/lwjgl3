    ...
    par_shapes_mesh* cylinder, int slices)
{
    ...
    scene->points = PAR_REALLOC(float, scene->points, npoints * 3);
    float* newpts = scene->points + scene->npoints * 3;
    ...
    scene->triangles = PAR_REALLOC(PAR_SHAPES_T, scene->triangles, ntriangles * 3);
    int v = scene->npoints - (slices + 1);
    PAR_SHAPES_T* face = scene->triangles + scene->ntriangles * 3;
    for (int stack = 0; stack < stacks; stack++) {
        for (int slice = 0; slice < slices; slice++) {
            int next = slice + 1;
            *face++ = v + slice + slices + 1;
